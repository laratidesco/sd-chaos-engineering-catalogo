"""PodChaos driver for the api-gateway-naive variant: kills one product-service
pod under active load and measures the time Kubernetes takes to detect and
recover from it, with no client-side retry cushioning the gap."""
...
MANIFEST = Path(__file__).parent.parent / "k8s" / "chaos" / "naive" / "pod-chaos-naive.yaml"
RESULTS_DIR = Path(__file__).parent / "results-naive" / "pod-chaos"
...
def sample():
    pods = []
    try:
        data = lib.kubectl_json("get", "pods", "-l", "app=product-service-naive")
...
def run():
    ...
    cpu = lib.prom_series_to_points(
        lib.query_prometheus_range(lib.cpu_query("product-service-naive"), t_start, t_end), t_start
    )
    mem = lib.prom_series_to_points(
        lib.query_prometheus_range(lib.memory_query("product-service-naive"), t_start, t_end), t_start
    )
    errors = lib.prom_series_to_points(
        lib.query_prometheus_range(lib.error_rate_query("api-gateway-naive"), t_start, t_end), t_start
    )
    lib.plot_timeseries([cpu], "CPU — product-service-naive", "cores", charts_dir / "cpu.png", attack_window)
    lib.plot_timeseries([mem], "Memória — product-service-naive", "bytes", charts_dir / "memory.png", attack_window)
    lib.plot_timeseries(
        [errors], "Taxa de erro (4xx/5xx) — api-gateway-naive", "fração de requisições",
        charts_dir / "error_rate.png", attack_window,
    )

    events = {
        "experiment": "pod-chaos-naive",
        "t_start": t_start,
        "t_attack": t_attack,
        "t_delete": t_delete,
        "t_end": t_end,
