...
back-to-back, then writes a consolidated summary (summary.json/summary.md)."""
...
import run_network_chaos
import run_pod_chaos
import run_stress_chaos
...
def capture_baseline():
    ...
    t_start = time.time()
    k6_proc, k6_log = lib.start_k6(BASELINE_SECONDS, baseline_dir)
    ...
    try:
        ready, desired = lib.get_deployment_ready("product-service")
...
def write_summary(baseline, results):
    ...
    lines = ["# Resumo dos Experimentos de Caos", ""]
    lines.append("## Estado Estável (baseline)")
    lines.append("")
    for job in ("api-gateway", "product-service"):
        b = baseline.get(job, {})
        lines.append(
...
def main():
    ...
    gw_pf = lib.start_port_forward("svc/api-gateway", 8080, 8000)
    ...
    try:
        ...
        lib.wait_steady_state(timeout=60)
        ...
        results = []
        for name, module in (
            ("network-chaos", run_network_chaos),
            ("pod-chaos", run_pod_chaos),
            ("stress-chaos", run_stress_chaos),
        ):
            gw_pf = lib.ensure_port_forward_alive(gw_pf, "svc/api-gateway", 8080, 8000)
        ...
        print("Done. See load-test/results/summary.md", flush=True)
    finally:
        gw_pf.terminate()
        prom_pf.terminate()
